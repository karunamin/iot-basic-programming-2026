...
- 컴파일러 : IDE(Integrity Development Environment) 툴을 사용
    ...
    - 컴파일러 - C/C++ 컴파일러, JAVA 컴파일러, C# 등 언어자체를 컴파일하는 프로그램
...
4. 함수
    - 함수 기본 : [확인](./Day03/Chap07-01-app/Chap07-01-app/main.c)
...
1. 함수 계속
    - 함수 유형 : [확인](./Day03/Chap07-Solution/Chap07-02-app/main.c)
        - 메개변수 없는 함수
        - 반환값이 없는 함수
        - 메개변수와 반환값이 없는 함수 - `void`
        - `메개변수, 반환값이 있는함수` - Chap07-01-app
    - 재귀호출 함수 : [확인](./Day04/Chap07-Solution/Chap-07-03-app/main.c)

2. 배열
    - 동일 자료형 묶음 - `ary[10]`
    - 배열 선언, 초기화 : [확인](./Day04/Chap08-Solution/Chap08-01-app/main.c)
    - 문자배열(문자열) : [확인](./Day04/Chap08-Solution/Chap08-03-app/main.c)
    - 대소문자 변환 : [확인](./Day04/Chap08-Solution/Chap08-quiz/mian.c)
...
1. **포인터**
    ...
    - 포인터 사용 이유 : [확인](./Day05/Chap09-Solution/Chap09-05-app/main.c)
        ...
        - 함부러 변수를 마구 찍어서 만들 수 없음!
        - 주소를 공유해서 함수어디서나 변수의 값을 변경할 수 있음
        - 함수간에 효과적으로 데이터를 공유하는 것
...
4. 문자열과 포인터 : [소스]
    ...
    - strscpn() : 문자열에서 찾고자 하는 문자열에 포함된 문자를 검색, 일치할때까지 읽어드린 문자의 갯수 리턴
...
3. 메모리 동적할당
    ...
    - `reallo` - re-allocation. 메모리 크기 조정
    - `free` - 메모리 반환
...
6. main 함수 명령해 인수 사용 : [소스](./Day07/Chap16-Solution/Chap16-06-app/main.c)
...
3. 파일 입출력
    ...
    - 바이너리 파일 : 데이터파일, 내부적으로 0과 1로 저장된 파일. mp3, dat, exe, mp3, mp4...
...
2. 폴더 및 파일경로
    ...
    - 상대경로 - `.\LICENSE.rrtf`와 같이, .(현재폴더), ..(부모폴더) 기호를 사용해서 짧게 경로를 표시방법
        - 장점 - 경로를 짧게 표시가능
        - 단점 - 개발자가 직접 경로가 어떤식으로 구성되는지 파악필요. 같은 폴더에 존재하는 경우 사용가능
...
3. 파일 입출력 : [소스](./Day09/Chap18-Solution/Chap18-01-app/main.c)
    ...
    - fgetc() - 파일에서 글자 하나씩 읽는 함수. 파일의 마지막(END Of File)까지 하나씩 읽는 함수
    - fputc() - 파일에 글자를 한자씩 출력(쓰는)하는 함수
    - fseek(), rewind() - 파일에 처음으로 돌아가서다시 읽기
    - feof() - 파일에 끝까지 다 일겅ㅆ는지 확인 함수
    - `fgets()` - 파일의 커서(현재위치)에서 한줄을 읽는함수
    - `fputs()` - 파일에 한줄을 출력(쓰는) 함수
    - fscanf() - 포맷에 맞춰서 파일에서 읽기 함수
    - fprintf() - 포맷에 맞춰서 파일에 쓰기 함수
    - stdio.h - stdin(표준입력), stdout(표준출력), stdeer(표준에러) 포함
    - 인코딩방식 - EUC-KR(CP-949, 멀티바이트, 한글만 표현, 2byte), `UTF-8`(한글자 최대00 6byte. 한글은 3byte)
...
5. 한글 깨짐 현상 [소스](./Day09/Chap18-Solution/Chap18-02-app/main.c)
    - Euc-KR 과 UTF-8 사이에서 변환문제들로 발생
    - 윈도우 설정 - `intl.cpl` 국가 또는 지역설정 윈도우 설정
...
6. 파일 핸들링 현산 : 
    - fopen() 이후 fclost() 이전에 탐색기 등에서 파일에 접근하면 파일을 열 수 없음

    ![alt text](image-18.png)

    - fseek() 함수 상용시 : [소스](./Day09/Chap18-Solution/Chap18-03-app/main.c) 
        - SEEK_SET - 파일의 최초 위치
        - SEEK_CUR - 파일의 현재 위치
        - SEEK_END - 파일의 마지막 위치

7. 전처리 지시자 : [소스](./Day09/Chap19-Solution/Chap19-01-app/main.c)
    - #include - 파일 포함 지시자. 내장 헤더파일은 <>로, 사용자헤더파일은""로
    - #define - 새로 정의하는 지시자. 자주 사용될 상수값을 정의할때 많이 사용. 주로 대문자로 지정
...
8. 분할 컴파일 : [소스](./Day09/Chap19-Solution/Chap19-04-app/main.c)
    - 여러파일을 분할해서 팀프로젝트나 프로그램 크기 줄이기, 코드 재활용 등
    - 전역변수 static - 자신의 소스코드 외부에 공개하지않는 내부용 정적 변수
    - 전역변수에 static은 사용하지 말것
