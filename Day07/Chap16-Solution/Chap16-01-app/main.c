...
int main(void) {
	...
	pi = (int*)malloc(sizeof(int));		// 1. 메모리 동적 할다 후 포인터와 연결, 4byte를 생성
	if (pi == NULL) {
	...
	*pi = 10;	// 메모리 주소 어딘가에 동적으로 int할당한 곳에 값을 10을 대입하라
	*pd = 3.4;

	printf("정수형 : %d\n", *pi);
	printf("실수형 : %.1f\n", *pd);

	free(pi);	// 2. 메모리를 동적 할당한 것은 종료직전 무조건 free로 메모리를 반환!
	free(pd);
