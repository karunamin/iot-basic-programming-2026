...
} Contect;	// 이후 Contect 이름으로 사용

#pragma region 변수 영역


// 변수 선언
static Contect contacts[MAX_CONTACTS];
static int count = 0;
...
static void serach_contact(void);
...
static void update_contact(void) {
	...
	puts("현재 정보:");
	print_contact(index - 1);	// 실제 인덱스는 0,1,2지만 검색순번은 1,2,3 을 사용하기때문에 -1
	puts("----------------------------------------");
...
static void delete_contact(void){
	...
	if (!(yn[0] == 'y' || yn[0] == 'Y')) {	// 소문자 y거나 대문자 Y가 아니면
		puts("삭제 취소합니다.");
		return;
	}
...
static int load_contacts(const char* filename) {
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		contacts[count].phone[PHONE_LEN - 1] = '\0';	// 전화번호길이보다 길게 쓰면 정리

		strncpy(contacts[count].address, address, ADDR_LEN);
		contacts[count].address[ADDR_LEN - 1] = '\0';	// 전화번호길이보다 길게 쓰면 정리

		strncpy(contacts[count].email, email, EMAIL_LEN);
		contacts[count].email[EMAIL_LEN - 1] = '\0';	// 전화번호길이보다 길게 쓰면 정리

		strncpy(contacts[count].memo, memo, MEMO_LEN);
		contacts[count].memo[MEMO_LEN - 1] = '\0';	// 전화번호길이보다 길게 쓰면 정리
